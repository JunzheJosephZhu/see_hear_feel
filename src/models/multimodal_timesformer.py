...
if __name__ == "__main__":
    ...
    args = p.parse_args()
    model = MultiModal_TimeSformer(image_size=(args.resized_height_v, args.resized_width_t), tactile_size=(args.resized_height_t, args.resized_width_t), patch_size=args.patch_size, num_stack=args.num_stack, frameskip=args.frameskip, fps=10, last_layer_stride=args.last_layer_stride, num_classes=3 ** args.action_dim, dim=args.dim, depth=args.depth, qkv_bias=args.qkv_bias, heads=args.heads, mlp_ratio=args.mlp_ratio, ablation=args.ablation, channels=3, audio_channels=2).cuda()
