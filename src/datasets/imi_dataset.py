...
class ImitationEpisode(EpisodeDataset):
    def __init__(self, log_file, args, dataset_idx, data_folder="data/test_recordings_0214", train=True):
        ...
        if self.train:
            self.transform_cam = [T.Resize((self.resized_height_v, self.resized_width_v)),]
            self.transform_gel = [T.Resize((self.resized_height_t, self.resized_width_t)),]
            
            if not args.no_jitter:
                self.transform_cam.append(T.ColorJitter(brightness=0.2, contrast=0.02, saturation=0.02))
                self.transform_gel.append(T.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2))
    ...
    def __getitem__(self, idx):
        ...
        # random cropping
        if self.train:
            img = self.transform_cam(self.load_image(self.trial, "cam_fixed_color", idx))
            if not self.nocrop:
            ...
            if "t" in self.modalities:
                if not self.use_flow:
                    img_t = self.transform_gel(self.load_image(self.trial, "left_gelsight_frame", idx))
                    if not self.nocrop:
