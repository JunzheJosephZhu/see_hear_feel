...
class ImmitationDataSet_Tuning(IterableDataset):
    ...
    def __next__(self):
        ...
        if len(self.framestack_cam) >= self.num_stack * self.frameskip:
            cam_framestack = self.framestack_cam[0]
            cam_fixed_framestack = self.framestack_cam[0]
            gs_framestack = self.framestack_gel[0]
            for i in range(1, self.num_stack):
                cam_framestack = np.vstack((cam_framestack, self.framestack_cam[i * self.frameskip]))
                cam_fixed_framestack = np.vstack((cam_fixed_framestack, self.framestack_fixed_cam[i * self.frameskip]))
                gs_framestack = np.vstack((gs_framestack, self.framestack_gel[i * self.frameskip]))
        elif len(self.framestack_cam) < self.num_stack * self.frameskip:
            cam_framestack = self.framestack_cam[-1]
            cam_fixed_framestack = self.framestack_cam[-1]
            gs_framestack = self.framestack_gel[-1]
            for i in range(1, self.num_stack):
                cam_framestack = np.vstack((cam_framestack, self.framestack_cam[-1]))
                cam_fixed_framestack = np.vstack((cam_fixed_framestack, self.framestack_fixed_cam[-1]))
                gs_framestack = np.vstack((gs_framestack, self.framestack_gel[-1]))
        # load audio clip
        # audio length is 1 second
        audio_start = self.timestep * resolution - sr // 2
        audio_end = audio_start + sr
        ...
        return cam_framestack, cam_fixed_framestack, gs_framestack, log_spec, action, pose
...
if __name__ == "__main__":
    ...
    dataset = ImmitationDataSet_Tuning("train.csv")
    # print("dataset", dataset.len)
    for cam_frame, _, _, _, _, _ in dataset:
        print(cam_frame)