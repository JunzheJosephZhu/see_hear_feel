...
class ImmitationDataSet(IterableDataset):
    def __init__(self, log_file=None, data_folder="data/test_recordings_0214"):
        super().__init__()
        ...
        self.pose_mean = torch.Tensor([.47, 0.0, 0.10])
        self.pose_var = torch.Tensor([0.08, 0.04, 0.04])
    ...
    def __next__(self):
        ...
        if self.timestep >= len(self.timestamps["action_history"]):
