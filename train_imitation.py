...
def main(args):
    ...
    train_set = torch.utils.data.ConcatDataset([ImitationEpisode(args.train_csv, args, i, args.data_folder) for i in range(train_num_episode)])
    val_set = torch.utils.data.ConcatDataset([ImitationEpisode(args.val_csv, args, i, args.data_folder, False) for i in range(val_num_episode)])
