...
class ImitationDatasetFramestack(BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        cam_gripper_framestack = torch.stack(
            [T.functional.crop(transform(self.load_image(self.trial, "cam_gripper_color", timestep)), i, j, h, w) for
             timestep in cam_idx], dim=0)
...
class ImitationDatasetFramestackMulti(BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        tactile_framestack = torch.stack(
            [T.functional.crop(transform(self.load_image(self.trial, "left_gelsight_frame", timestep)), i, j, h, w) for
             timestep in cam_idx], dim=0)
        # print(time.time() - t_start)
        ...
        return cam_fixed_framestack, tactile_framestack, log_spec, keyboard
...
class ImitationDataSet_hdf5_multi(IterableDataset):
    ...
    def load_episode(self, idx):
        ...
        trial = os.path.join(self.data_folder, format_time)
        # load audio tracks
        self.audio = torch.as_tensor(
            np.stack([self.all_datasets['audio_gripper'], self.all_datasets['audio_holebase']], 0)).float()
        # load json file
        with open(os.path.join(trial, "timestamps.json")) as ts:
            self.timestamps = json.load(ts)
        # read all data
