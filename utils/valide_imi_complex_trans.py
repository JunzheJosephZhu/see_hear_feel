...
from src.datasets.transformer_dataset import TransformerDataset
...
from src.models.multimodal_timesformer import MuT
...
def baselineValidate(args):
    ...
    if args.episode is None:
        val_set = torch.utils.data.ConcatDataset(
            [
                TransformerDataset(args.val_csv, args, i, args.data_folder, False)
                for i in range(len(val_csv))
            ]
        )
    else:
        val_set = TransformerDataset(
            ...
            for i in range(len(titles)):
                if i < 1:
                    ...
                    if keyboard_3[i - 1] == pred_action_3[i - 1]:
                        ...
                        mycolor = "r"
                    video_saver.axs_seq[i].set_title(
                        f"({titles[i]}) gt: {keyboard_3[i-1]}, pred: {pred_action_3[i-1]}",
                        color=mycolor,
                        fontsize=8,
                    )
                    ...
                    video_saver.axs_seq[i].plot(
                        cnt - 1, keyboard_3[i - 1], "b+", label="real"
                    )
                    video_saver.axs_seq[i].plot(
                        cnt - 1, pred_action_3[i - 1], "r.", label="predict"
                    )
                # axs[i].legend()
