...
class MakeVideo():
    def __init__(self, dir=None, framestack=None, name='default', num_episode=1, mods='v_t'):
        ...
        self.mydict = {
            ...
            't': {'resolution': (100, 100),
                  'font_scale': .7,
                  'thick_scale': 1},
            't_final': {'resolution': (160 * framestack, 280),
                  'font_scale': .7,
                  'thick_scale': 1},
        }
        self.path_dict = {'v': [],
    ...
    def initialize_sep(self):
        ...
        for i in range(self.framestack):
            ...
            # self.video_writer["v_final_video"] = cv2.VideoWriter(
            #     os.path.join(self.dir, 'camframe_final.avi'),
            #     cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'),
            #     10,
            #     self.mydict['v_final']['resolution']
            # )
            # self.video_writer["t_final_video"] = cv2.VideoWriter(
            #     os.path.join(self.dir, 'tac_final.avi'),
            #     cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'),
            #     10,
            #     self.mydict['t_final']['resolution']
            # )
...
def baselineValidate(args):
    ...
    for label in range(3 ** args.action_dim):
        if pred_label_cnts[label] > 0 or real_label_cnts[label] > 0:
            print(f"plot label {label}, real cnt {real_label_cnts[label]}, pred cnt {pred_label_cnts[label]}")
            rect = patches.Rectangle(xy=(label, 0), width=.5, height=real_label_cnts[label], edgecolor='None', color='b')
            axs.add_patch(rect)
            rect = patches.Rectangle(xy=(label+.5, 0), width=.5, height=pred_label_cnts[label], edgecolor='None', facecolor='r')
            axs.add_patch(rect)
            max_label = max(real_label_cnts[label], pred_label_cnts[label])
            axs.text(x=label, y=-5, s=str(label), fontsize=7)
            action = label_to_action(label)
