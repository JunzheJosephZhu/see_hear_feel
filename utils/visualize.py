...
class Tests:
    ...
    def test_confusion_matrix(self):
        ...
        for s in tqdm(f[self.test_item].iter_chunks()):
            ...
            sn.set(font_scale=1)
            img = sn.heatmap(
                df_cm, annot=True, cmap="YlGnBu", annot_kws={"fontsize": 10}
            ).get_figure()
    ...
    def test_audio(self):
        ...
        if self.store:
            ...
            # audio_buffer *= 10
            fs = 44100
            sd.write(file=self.path + ".wav", data=audio_buffer, samplerate=fs)
        else:
            plt.figure()
            sr = 44100
            ...
            audio_buffer_arr = np.clip(audio_buffer_arr, a_min=0, a_max=0.5)
...
class Analysis:
    ...
    def _save_vt_video(self):
        ...
        comp_clip = mpe.clips_array(
            [[v_list[0], t_list[0]], [v_list[1], t_list[1]], [v_list[2], t_list[2]]]
        )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--data_folder",
        default="../testing_models/key_insertion/exp05042022/imi_learn_ablation_v/checkpoints/ep0",
    )
    args = p.parse_args()
